...
Residuals::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   */
   ...
   for(TrackCollection::const_iterator itk = tracks->begin(); itk != tracks->end(); ++itk){
     ...
     raw.nMissedOut = itk->hitPattern().numberOfLostHits(HitPattern::MISSING_OUTER_HITS);
     raw.nMissedIn  = itk->hitPattern().numberOfLostHits(HitPattern::MISSING_INNER_HITS);
     raw.hasPXL     = (itk->hitPattern().hasValidHitInPixelLayer(PixelSubdetector::SubDetector::PixelBarrel, 1) || 
